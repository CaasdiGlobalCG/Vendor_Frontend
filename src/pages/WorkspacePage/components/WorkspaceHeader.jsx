...
const WorkspaceHeader = ({
 ...
 const handleBackToDashboard = () => {
    ...
    switch (syncStatus) {
      ...
      case 'offline':
        return {
          ...
          dotClass: 'bg-danger',
          containerClass: 'border-danger/20 bg-danger/10 text-danger',
        };
      case 'error':
        return {
          label: 'Sync issue',
      ...
      default:
        return {
          ...
          dotClass: 'bg-cta',
          containerClass: 'border-line bg-surface-hover text-ink',
        };
    }
  };
  ...
  return (
    <div className="bg-surface border-b border-line px-2 py-1.5" data-workspace-header>
      <div className="relative flex items-center justify-between gap-2 overflow-hidden">
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3 lg:gap-4">
          <button 
            ...
            className="flex shrink-0 items-center space-x-2 rounded-lg px-2.5 py-2 text-ink transition-colors hover:bg-surface-hover"
          ...
          <div className="flex shrink-0 items-center space-x-2">
            <div className="flex items-center space-x-2 cursor-pointer">
              <div className="w-8 h-8 bg-black rounded-lg flex items-center justify-center text-white font-medium text-sm">
                CG
              </div>
            ...
            {isWorkspaceCompleted && isVendor && (
              <button
                ...
                className="flex items-center space-x-1 px-2 py-1 text-xs text-dim hover:text-ink hover:bg-surface-hover rounded transition-colors"
        ...
        <div className="flex shrink-0 items-center space-x-1 md:space-x-1.5">
          ...
          {showPostServicesActions && (
            <>
              <button
                ...
                className="flex items-center space-x-1 rounded-lg p-1.5 transition-colors hover:bg-surface-hover"
                title="Post Services"
              >
                <MessageCircle className="w-4 h-4 text-dim" />
                <span className="hidden 2xl:inline text-[10px] font-medium text-dim">Post Service</span>
              </button>
            </>
          )}
          ...
          {showPostServicesActions && (
            <div ref={overflowRef} className="relative">
              ...
              {showOverflowMenu && (
                <div className="absolute right-0 top-full mt-1 w-48 bg-surface border border-line rounded-lg shadow-lg z-50 py-1">
                  {isPM ? (
                    <>
                      <button onClick={() => { onOpenReviewProgress(); setShowOverflowMenu(false); }} className="w-full flex items-center space-x-2 px-3 py-2 text-sm text-ink hover:bg-canvas">
                        <Zap className="w-4 h-4 text-warning" />
                        <span>Review Progress</span>
                      </button>
                      <button onClick={() => { onOpenProjectComplete(); setShowOverflowMenu(false); }} className="w-full flex items-center space-x-2 px-3 py-2 text-sm text-ink hover:bg-canvas">
                    ...
                    <>
                      <button onClick={() => { onOpenClientReviewProgress(); setShowOverflowMenu(false); }} className="w-full flex items-center space-x-2 px-3 py-2 text-sm text-ink hover:bg-canvas">
                    ...
                    <button
                      ...
                      className={`w-full flex items-center space-x-2 px-3 py-2 text-sm ${shouldDisableEditing ? 'text-dim cursor-not-allowed' : 'text-ink hover:bg-canvas'}`}
                    >
                  ...
                  <button
                    ...
                    className="w-full flex items-center space-x-2 px-3 py-2 text-sm text-ink hover:bg-canvas"
                  >
          ...
          <button
            ...
            className="p-1.5 hover:bg-surface-hover rounded-full transition-all duration-200 group border border-line hover:border-line bg-black hover:from-black hover:to-black"
            title="Workspace Tutorial"
          >
            <HelpCircle className="w-4 h-4 text-ink group-hover:scale-110 transition-transform" />
          </button>
          ...
          <button
            ...
            className="hidden xl:flex items-center gap-1.5 rounded-lg border border-line bg-canvas px-2 py-1 text-xs text-dim transition-colors hover:bg-surface-hover"
            title="Open command palette (Ctrl+K)"
          >
            <Search className="w-3 h-3" />
            <span className="text-[10px]">Search</span>
            <kbd className="ml-0.5 px-1 py-0.5 text-[9px] bg-surface border border-line rounded">⌘K</kbd>
          </button>
        </div>
      </div>
      ...
      {showTutorial && (
        <>
          ...
          >
            ...
            {arrowPosition === 'top' && (
              <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-surface rotate-45 shadow-lg" />
            )}
            {arrowPosition === 'bottom' && (
              <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-surface rotate-45 shadow-lg" />
            )}
            {arrowPosition === 'left' && (
              <div className="absolute top-1/2 -translate-y-1/2 -left-2  w-4 h-4 bg-surface rotate-45 shadow-lg" />
            )}
            {arrowPosition === 'right' && (
              <div className="absolute top-1/2 -translate-y-1/2 -right-2  w-4 h-4 bg-surface rotate-45 shadow-lg" />
            )}
            {arrowPosition === 'top-right' && (
              <div className="absolute -top-2 right-8 w-4 h-4 bg-surface rotate-45 shadow-lg" />
            )}
            
            <div className="bg-surface rounded-xl shadow-2xl overflow-hidden border border-line">
              {/* Header */}
              ...
              <div className="px-4 pb-4 flex justify-between items-center">
                <button
                  ...
                  className={`flex items-center space-x-1 px-3 py-1.5 rounded-lg text-sm transition-all ${
                    currentStep === 0
                      ? 'text-dim cursor-not-allowed'
                      : 'text-dim hover:bg-surface-hover'
                  }`}
                >
                ...
                {currentStep === tutorialSteps.length - 1 ? (
                  <button
                    onClick={closeTutorial}
                    className="flex items-center space-x-1 px-4 py-1.5 bg-black text-white rounded-lg text-sm hover:from-black hover:to-black transition-all "
                  >
                    <span>Finish</span>
