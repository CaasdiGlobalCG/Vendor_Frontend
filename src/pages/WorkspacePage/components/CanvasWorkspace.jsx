...
const edgeTypes = {
  ...
  const containerClasses = isFullscreen
    ? 'fixed left-0 right-0 bottom-0 bg-surface z-50 flex flex-col'
  ...
  return (
    <>
      ...
      {/* Subtask Header */}
      <div className="pl-20 pr-6 py-3 md:pl-20 md:pr-8 bg-white/20 supports-[backdrop-filter]:bg-white/10 backdrop-blur-xl border-b border-white/30 ">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
          <h2 className="text-lg font-semibold text-ink">
              {selectedSubtask?.name || workspace?.title || 'Workspace'}
            </h2>
            <p className="text-dim mt-1">Canvas workspace for {selectedSubtask?.name || workspace?.title || 'your project'}</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 relative z-20 md:mt-0">
            ...
            {canvasWebSocket && (
              <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-white/40 supports-[backdrop-filter]:bg-white/20 backdrop-blur-md border border-white/40  text-xs">
                <div className={`w-2 h-2 rounded-full ${canvasWebSocket.isConnected ? 'bg-success' : 'bg-danger'}`} />
            ...
            <button 
              ...
              className={`p-2 rounded-lg border border-info  backdrop-blur-md transition-colors flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-info ${
                saveStatus === 'saving' 
                  ? 'bg-cta cursor-not-allowed text-cta-foreground' 
                  : 'bg-info hover:bg-info text-cta-foreground'
              }`}
      ...
      >
        ...
        <ReactFlow
          ...
          className={`bg-transparent transition-all duration-200 ${
            isDraggingOver ? 'bg-info ring-4 ring-info/30' : ''
          }`}
        >
          ...
          {aiReview && (
            <Panel position="top-center" className="mt-4">
              <div className="flex items-center gap-3 bg-surface shadow-2xl border border-line rounded-xl px-4 py-2.5">
          ...
          <Panel position="top-right" className="mt-4 mr-4">
            <div className="bg-surface shadow-lg rounded-lg px-4 py-2 border border-line">
            <div className="flex flex-wrap items-center gap-4">
                {/* <div className="text-xs text-dim space-y-1 max-w-xs">
                   ...
                   <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors border ${
                        isSelectionMode
                          ? 'bg-info text-white hover:bg-info border-info'
                          : 'bg-info/10 text-info hover:bg-info/10 border-info/20'
                      }`}
          ...
          {showFlowchartToolbar && selectedFlowchartGroup && (
            <Panel position="top-right">
              <div className="bg-surface shadow-lg rounded-lg border border-line p-4 min-w-[250px]">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-ink">Extend Flowchart</h3>
                  <button
                    onClick={() => {
                      setShowFlowchartToolbar(false);
                      setSelectedFlowchartGroup(null);
                ...
                <div className="space-y-2">
                  <p className="text-xs text-dim mb-3">Add new elements to your flowchart:</p>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      <div className="w-3 h-3 bg-cta rounded"></div>
                      <span>Text</span>
                    </button>
                  </div>
          ...
          {isDraggingOver && (
            <Panel position="center">
              <div className="text-center pointer-events-none">
                <div className="w-32 h-32 bg-info/10 rounded-full flex items-center justify-center mx-auto mb-6 animate-pulse">
                  <Plus className="w-16 h-16 text-info" />
                </div>
                <h3 className="text-2xl font-bold text-info mb-2">Drop Here!</h3>
                <p className="text-info mb-6">Release to add element to canvas</p>
              </div>
            </Panel>
          )}
          ...
          {nodes.length === 0 && !isDraggingOver && (
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center space-y-2.5">
                <div className="w-10 h-10 bg-surface-hover rounded-full flex items-center justify-center mx-auto opacity-50">
                  <Plus className="w-8 h-8 text-dim" />
                </div>
                <div className="space-y-0.5">
                  <h3 className="text-lg font-semibold text-dim">Start Creating</h3>
                  <p className="text-dim text-xs">Drag elements from the Elements panel</p>
                  <p className="text-dim text-[11px]">or double-click any element to add it</p>
                </div>
                {/* <h3 className="text-2xl font-bold text-dim mb-2">Start Creating</h3>
                <p className="text-dim mb-2">Drag elements from the Elements panel</p>
                <p className="text-dim text-sm">or double-click any element to add it</p>
              </div> */}
              </div>
            {/* </Panel> */}
            </div>
          ...
          {canEdit && nodes.length > 0 && (
            <Panel position="bottom-left" className="pointer-events-none">
              <div className="bg-white/90 backdrop-blur-sm rounded-lg p-3 border border-line ">
                <div className="text-xs text-dim space-y-1">
                  <div className="flex items-center space-x-2">
                    <kbd className="px-1.5 py-0.5 bg-surface-hover rounded text-xs">Ctrl+D</kbd>
                    <span>Duplicate selected</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <kbd className="px-1.5 py-0.5 bg-surface-hover rounded text-xs">Right-click</kbd>
                    <span>Context menu</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <kbd className="px-1.5 py-0.5 bg-surface-hover rounded text-xs">Del</kbd>
        ...
        {!canEdit && userRole !== 'client' && (
          <div className="absolute inset-0 bg-black bg-opacity-5 pointer-events-none z-10 flex items-center justify-center">
            <div className="bg-surface rounded-lg shadow-lg p-4 border border-line pointer-events-auto">
              <div className="flex items-center space-x-3">
                <div className="flex-shrink-0">
                  <svg className="h-8 w-8 text-dim" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                  </svg>
                </div>
                <div className="space-y-1">
                  <h3 className="text-xs font-semibold text-ink uppercase tracking-wide">Canvas View Only</h3>
        ...
        {!canEdit && userRole === 'client' && (
          <div className="absolute top-4 right-4 z-10 bg-canvas border border-line rounded-lg px-3 py-2 flex items-center space-x-2 text-xs text-dim">
      ...
      {duplicateToAllState.isVisible && (
        <div className="fixed inset-0 z-[90] bg-black/40 backdrop-blur-[1px] flex items-center justify-center px-4">
          <div className="w-full max-w-md bg-surface rounded-xl shadow-2xl border border-line p-5">
            <div className="flex items-start gap-3">
              <div className={`mt-0.5 w-8 h-8 rounded-full flex items-center justify-center ${duplicateToAllState.isLoading ? 'bg-info/10' : 'bg-surface-hover'}`}>
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-dim mt-1">{duplicateToAllState.message}</p>
              </div>
            </div>
      ...
      {edgeLabelModal.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-surface rounded-xl shadow-2xl w-full max-w-sm p-4">
            {/* Header */}
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-base font-semibold text-ink">Edit Connection</h3>
              <button
            ...
            {/* Connection Name */}
            <div className="mb-3">
              <label className="block text-xs font-medium text-ink mb-1">Connection Name</label>
              <input
                type="text"
                autoFocus
            ...
            {/* Connection Style */}
            <div className="mb-3">
              <label className="block text-xs font-medium text-ink mb-1.5">Line Style</label>
                  ...
                  <button
                    ...
                    className={`px-2 py-1.5 rounded-lg border-2 text-xs font-medium transition-all ${
                      edgeStyleInput === style.id
                        ...
                        : 'border-line hover:border-line text-dim'
                    }`}
                  >
            ...
            {/* Connection Color */}
            <div className="mb-4">
              <label className="block text-xs font-medium text-ink mb-1.5">Line Color</label>
                  ...
                  <button
                    ...
                    className={`w-7 h-7 rounded-full border-2 transition-all ${
                      edgeColorInput === c.color
                        ? 'border-line ring-2 ring-offset-2 ring-info'
      ...
      {showClearConfirmation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-surface rounded-lg shadow-xl w-full max-w-md overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-line bg-danger/10">
              <h3 className="text-lg font-semibold text-danger">Clear All Elements?</h3>
              <button
