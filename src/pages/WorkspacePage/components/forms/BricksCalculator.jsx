...
const BricksCalculator = ({ data, nodeId, workspaceId, setNodes }) => {
  ...
  const renderBrickTypeStep = () => (
    <div className="p-6 space-y-4">
      {/* Title */}
      <div className="mb-6">
        <h3 className="text-xl font-bold text-ink">Bricks Calculator</h3>
        <p className="text-sm text-dim mt-1">Step 1: Select or define brick type</p>
      </div>

      {/* Preset Brick Types */}
      <div className="space-y-3">
        <label className="block text-sm font-medium text-ink">Standard Brick Types</label>
        <div className="grid grid-cols-2 gap-3">
          {Object.entries(presetBrickTypes).map(([key, brick]) => (
            ...
            >
              <p className="font-semibold text-sm text-ink">{brick.name}</p>
              <p className="text-xs text-dim mt-1">{brick.description}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="relative flex items-center my-4">
        ...
        <span className="px-4 text-sm text-dim">OR</span>
        <div className="flex-grow border-t border-line"></div>
      </div>
      ...
      <div className="space-y-3">
        ...
        >
          <p className="font-semibold text-sm text-ink">Custom Brick</p>
          <p className="text-xs text-dim mt-1">Enter your own dimensions</p>
        </button>

        {selectedBrickType === 'custom' && (
          <div className="bg-canvas rounded-lg p-4 space-y-3 border-2 border-info/20">
            {/* Custom Brick Name */}
            <div className="space-y-1">
              <label className="block text-xs font-medium text-ink">Brick Name/Label</label>
              <input
                type="text"
                  ...
                  <button
                    ...
                    className={`flex-1 py-1.5 px-2 rounded-lg font-medium text-xs transition-all ${
                      brickDimensionUnit === unitOption
                        ? 'bg-info text-white'
                        : 'bg-surface border-2 border-line text-ink hover:bg-surface-hover'
                    }`}
                  >
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="space-y-1">
                ...
                <input
                  ...
                  step="0.1"
                />
                <p className="text-xs text-dim">{brickDimensionUnit}</p>
              </div>
              <div className="space-y-1">
            ...
            <p className="text-xs text-dim italic">Note: Only LENGTH × HEIGHT (face) are used in calculation. Depth is for reference.</p>
          </div>
        )}
      </div>
      ...
      <button
        ...
        className="w-full bg-info hover:bg-info text-white font-bold py-3 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center gap-2 mt-6"
      >
  ...
  const renderDimensionsStep = () => (
    <div className="p-6 space-y-4">
      ...
      <div className="mb-6">
        <h3 className="text-xl font-bold text-ink">Bricks Calculator</h3>
        <p className="text-sm text-dim mt-1">Step 2: Enter wall dimensions & settings</p>
      </div>

      {/* Selected Brick Summary */}
      <div className="bg-info/10 border-2 border-info/20 rounded-lg p-4 mb-4">
        <div className="text-sm">
          <p className="font-medium text-ink">Selected Brick:</p>
          <p className="text-ink mt-1">{customBrickName}</p>
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-ink">Wall dimensions in</label>
        <div className="flex gap-2">
          <button
      ...
      <div className="border-t border-line pt-4 mt-4">
        ...
        <p className="text-xs text-dim mt-2">Standard: 10mm for typical mortar joint</p>
      </div>
        ...
        <div className="bg-surface rounded-lg shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col">
          ...
          <div className="sticky top-0 bg-info/10 border-b border-info/20 p-4 flex items-center justify-between flex-shrink-0">
            <div>
              ...
              <p className="text-sm text-dim">Surveyor-grade brick count calculation</p>
            </div>
            <button
            ...
            <div className="border border-line rounded-lg overflow-hidden">
              ...
              <div className="bg-canvas p-4 border-b border-line">
                ...
                <div className="space-y-1 text-sm text-ink ml-8">
                  ...
                  <p className="text-dim mt-2">Rounded UP to nearest whole brick: <span className="font-bold">{calculations?.numBricksRounded} bricks</span></p>
                </div>
              </div>
              ...
              <div className="bg-surface p-4">
                ...
                <div className="space-y-1 text-sm text-ink ml-8">
                  ...
                  <p className="text-success font-bold mt-2">FINAL ANSWER: {result?.toLocaleString()} bricks</p>
                </div>
              </div>
            </div>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-success/10 border-2 border-success/20 rounded-lg p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <p className="text-sm text-dim mb-2">Bricks Required</p>
            <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-dim mt-2">bricks needed for your wall</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-info/10 border-2 border-info/20 rounded-lg p-4 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            <p className="text-dim">Brick Type</p>
            <p className="font-bold text-ink">{calculations?.brickName}</p>
          </div>
          <div>
