...
const MaterialsRenderer = ({ data, materialType, workspaceId, currentUser, nodeId }) => {
    ...
    switch (type) {
      ...
      case 'consumables':
        return {
          ...
          icon: Package,
          color: 'bg-surface-hover border-line text-ink',
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      ...
      default: return 'bg-surface-hover text-ink border-line';
    }
  };
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'submitted': return <CheckCircle className="w-4 h-4 text-success" />;
      case 'processing': return <Clock className="w-4 h-4 text-info" />;
      case 'draft': return <AlertCircle className="w-4 h-4 text-dim" />;
      default: return <AlertCircle className="w-4 h-4 text-dim" />;
    }
  };
  ...
  return (
    <div className="w-full space-y-4">
      ...
      {procurementStatus === 'sent' && (
        <div className="bg-success/10 border border-success/20 rounded-lg p-3 flex items-center space-x-3">
          <CheckCircle className="w-5 h-5 text-success flex-shrink-0" />
          <div className="flex-1">
            <p className="text-sm font-medium text-success">Sent to Procurement Team</p>
      ...
      <div className={`p-4 rounded-lg border-2 ${category.color}`}>
        ...
        <input
          ...
          className="w-full mt-2 px-3 py-2 bg-surface border border-line rounded-md focus:ring-2 focus:ring-info focus:border-info font-medium"
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="font-medium text-ink">Material Items ({materialRequests.length})</h4>
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-3 py-2 text-sm rounded-md transition-colors border ${
                procurementStatus === 'sent' && !isChangeRequestMode
                  ? 'bg-surface-hover text-dim border-line cursor-not-allowed' 
                  : 'bg-success/10 hover:bg-success/10 text-success border-success/20'
              }`}
              disabled={procurementStatus === 'sent' && !isChangeRequestMode}
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-3 py-2 text-sm rounded-md transition-colors border ${
                procurementStatus === 'sent' && !isChangeRequestMode
                  ? 'bg-surface-hover text-dim border-line cursor-not-allowed' 
                  : 'bg-info/10 hover:bg-info/10 text-info border-info/20'
              }`}
              disabled={procurementStatus === 'sent' && !isChangeRequestMode}
            >
        ...
        {materialRequests.map((request, index) => (
          <div key={request.id} className="bg-canvas rounded-lg p-4 border border-line space-y-3">
            {/* Item Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <span className="text-sm font-medium text-dim">Item {index + 1}</span>
                {getStatusIcon(request.status)}
                <span className="text-xs text-dim capitalize">{request.status}</span>
              </div>
              {procurementStatus !== 'sent' && (
                <button
                  onClick={(e) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {/* Material Name */}
              <div>
                <label className="block text-xs font-medium text-ink mb-1">Material Name *</label>
                <input
                  type="text"
                  value={request.name}
                  onChange={(e) => updateMaterialRequest(request.id, 'name', e.target.value)}
                  className={`w-full px-3 py-2 border border-line rounded-md focus:ring-2 focus:ring-info focus:border-info text-sm ${procurementStatus === 'sent' && !isChangeRequestMode ? 'bg-surface-hover' : ''}`}
              ...
              <div className="flex space-x-2">
                ...
                <div className="w-20">
                  ...
                  <select
                    ...
                    className={`w-full px-2 py-2 border border-line rounded-md focus:ring-2 focus:ring-info focus:border-info text-sm ${procurementStatus === 'sent' && !isChangeRequestMode ? 'bg-surface-hover' : ''}`}
                    onClick={(e) => e.stopPropagation()}
                    disabled={procurementStatus === 'sent' && !isChangeRequestMode}
                  >
            ...
            <div>
              ...
              <textarea
                ...
                className={`w-full px-3 py-2 border border-line rounded-md focus:ring-2 focus:ring-info focus:border-info text-sm resize-none ${procurementStatus === 'sent' && !isChangeRequestMode ? 'bg-surface-hover' : ''}`}
                rows="2"
      ...
      <div className="flex justify-center gap-3 pt-4">
        {procurementStatus === 'sent' && !isChangeRequestMode ? (
          <>
            <div className="flex items-center space-x-3 px-6 py-3 bg-success/10 text-success rounded-lg border border-success/30 flex-1">
              <CheckCircle className="w-5 h-5 text-success flex-shrink-0" />
            ...
            <button
              ...
              className="flex items-center space-x-2 px-6 py-3 bg-info/10 hover:bg-info/10 text-info border border-info/30 rounded-lg transition-colors font-medium"
          ...
          <>
            <button
              ...
              className="flex items-center space-x-2 px-6 py-3 bg-surface-hover hover:bg-surface-hover text-ink border border-line rounded-lg transition-colors font-medium"
            >
              <X className="w-5 h-5" />
            ...
            <button
              ...
              className="flex items-center space-x-3 px-6 py-3 bg-black text-white rounded-lg hover:from-black hover:to-black transition-all duration-200 font-medium"
            >
