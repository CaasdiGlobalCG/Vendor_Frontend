...
const BOQInput = ({ label, value, onChange, type = 'text', step = 'any' }) => (
  <div>
    ...
    <input type={type} value={value} onChange={onChange} step={step}
      ...
      autoComplete="off"
      className="w-full border border-line rounded-md px-3 py-2.5 text-sm bg-surface focus:outline-none focus:ring-2 focus:ring-info focus:border-transparent transition" />
  </div>
);
...
const BOQChk = ({ label, checked, onChange }) => (
  <label className="flex items-center gap-3 cursor-pointer py-1">
    <input type="checkbox" checked={checked} onChange={onChange} className="w-4 h-4 rounded accent-info" />
...
const BOQTab = ({ label, active, onClick }) => (
  <button onClick={onClick}
    className={`px-4 py-2.5 text-sm font-semibold whitespace-nowrap transition border-b-2 ${
      active ? 'border-info text-info bg-info/10' : 'border-transparent text-dim bg-surface hover:text-ink hover:bg-canvas'}`}>
    {label}
  </button>
);
      ...
      <div className="bg-surface rounded-xl w-[95vw] max-w-5xl max-h-[90vh] flex flex-col shadow-2xl"
        ...
        <div className="bg-black text-white px-6 py-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-info mt-1">Step {currentTabIndex + 1} of {tabOrder.length} • {(['Project Details', 'Excavation', 'Concrete', 'Steel', 'Masonry', 'Plaster', 'Flooring', 'Costs'][currentTabIndex])}</p>
          </div>
          <button onClick={onClose} className="hover:bg-info p-2 rounded-lg transition"><X size={22} /></button>
        </div>

        {/* Progress bar */}
        <div className="h-2 bg-info/10">
          <div 
        ...
        {/* Tabs */}
        <div className="bg-surface border-b border-line flex gap-0 px-2 py-0 overflow-x-auto">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-surface-hover">
          {activeTab === 'project' && (
            <div className="space-y-5">
              <div>
                <h3 className="text-sm font-bold text-ink mb-4">Project Information</h3>
              ...
              <div className="border-t pt-5">
                <h3 className="text-sm font-bold text-ink mb-4">Include Sections</h3>
        ...
        {/* Footer */}
        <div className="border-t border-line bg-canvas px-6 py-4 flex justify-between gap-3">
          <button onClick={onClose}
            className="px-4 py-2 rounded-lg font-semibold bg-surface-hover hover:bg-cta text-ink text-sm transition">
            Cancel
          </button>
          <div className="flex gap-3">
            {currentTabIndex > 0 && (
              <button onClick={handleBack}
                className="px-5 py-2 rounded-lg font-semibold bg-cta hover:bg-cta text-cta-foreground text-sm transition">
                ← Back
              </button>
            )}
            {!isLastTab ? (
              <button onClick={handleNext}
                className="px-6 py-2 rounded-lg font-semibold bg-info hover:bg-info text-cta-foreground text-sm transition  ">
                Next →
              </button>
            ) : (
              <button onClick={handleGenerate}
                className="px-6 py-2 rounded-lg font-semibold bg-black hover:from-black hover:to-black text-white text-sm flex items-center gap-2 transition ">
...
const BOQTableDisplay = ({ boqData, costBreakdown, onRegenerate }) => {
  ...
  return (
    <div className="w-full bg-surface rounded-lg  border border-line overflow-hidden" onClick={e => e.stopPropagation()}>
      {/* Compact Header */}
      <div className="bg-black text-white px-3 py-2 flex justify-between items-center">
        <div className="flex-1 min-w-0">
          ...
          {projectInfo.clientName && <p className="text-xs text-info truncate">{projectInfo.clientName}</p>}
        </div>
        <button onClick={onRegenerate} title="Edit BOQ"
          className="ml-2 hover:bg-info p-1 rounded transition-colors flex-shrink-0">
          <RefreshCw size={14} />
        </button>
      </div>
      ...
      <div className="overflow-auto" style={{ maxHeight: '600px' }}>
        <table className="w-full border-collapse min-w-[550px]">
          <thead className="sticky top-0">
            <tr className="bg-cta text-cta-foreground">
              <th className="border border-line px-1.5 py-1 text-left font-semibold text-xs w-10">S#</th>
              <th className="border border-line px-1.5 py-1 text-left font-semibold text-xs">Description</th>
              <th className="border border-line px-1.5 py-1 text-right font-semibold text-xs w-14">Qty</th>
              <th className="border border-line px-1.5 py-1 text-center font-semibold text-xs w-12">Unit</th>
              <th className="border border-line px-1.5 py-1 text-right font-semibold text-xs w-16">Rate</th>
              <th className="border border-line px-1.5 py-1 text-right font-semibold text-xs w-16">Amount</th>
            </tr>
          </thead>
          <tbody className="text-xs">
            {sections.map((section) => (
              <React.Fragment key={section.sectionNo}>
                {/* Section Header */}
                <tr className="bg-info/10 hover:bg-info/20">
                  <td colSpan={6} className="border border-line px-2 py-1 font-bold text-info">
                ...
                {section.items.map((item, idx) => {
                  ...
                  return (
                    <tr key={idx} className={`${serial % 2 === 0 ? 'bg-canvas' : 'bg-surface'} hover:bg-info/10`}>
                      <td className="border border-line px-1.5 py-0.5 text-center font-medium text-ink">{serial}</td>
                      <td className="border border-line px-1.5 py-0.5 text-ink">{item.description}</td>
                      <td className="border border-line px-1.5 py-0.5 text-right font-semibold text-ink">{(item.quantity || 0).toFixed(2)}</td>
                      <td className="border border-line px-1.5 py-0.5 text-center text-ink">{item.unit}</td>
                      <td className="border border-line px-1.5 py-0.5 text-right text-ink">{(item.rate || 0).toLocaleString('en-IN')}</td>
                      <td className="border border-line px-1.5 py-0.5 text-right font-semibold text-success">{amount.toLocaleString('en-IN')}</td>
                    </tr>
                  );
                })}
              </React.Fragment>
          ...
          <table className="w-full text-xs border-collapse">
            <tbody>
              <tr className="bg-surface border-b border-line">
                <td className="px-2 py-1 font-semibold text-ink col-span-5">Subtotal</td>
                <td className="px-2 py-1 text-right font-bold text-ink">₹{costBreakdown.itemsCost.subtotal.toLocaleString('en-IN')}</td>
              </tr>
              <tr className="bg-canvas border-b border-line">
                <td className="px-2 py-0.5 text-dim">Overhead ({costBreakdown.overhead.overheadPercent}%)</td>
                <td className="px-2 py-0.5 text-right text-ink">₹{costBreakdown.overhead.overheadAmount.toLocaleString('en-IN')}</td>
              </tr>
              <tr className="bg-canvas border-b border-line">
                <td className="px-2 py-0.5 text-dim">Contingency ({costBreakdown.overhead.contingencyPercent}%)</td>
                <td className="px-2 py-0.5 text-right text-ink">₹{costBreakdown.overhead.contingencyAmount.toLocaleString('en-IN')}</td>
              </tr>
              <tr className="bg-canvas border-b border-line">
                <td className="px-2 py-0.5 text-dim">GST ({costBreakdown.gst.gstPercent}%)</td>
                <td className="px-2 py-0.5 text-right text-ink">₹{costBreakdown.gst.gstAmount.toLocaleString('en-IN')}</td>
              </tr>
      ...
      {/* Export Footer */}
      <div className="bg-surface-hover px-2 py-2 flex justify-end gap-1.5 border-t border-line">
        <button onClick={() => boqAggregator.exportToExcel(boqData, costBreakdown)}
          className="px-2.5 py-1 rounded text-xs font-semibold bg-success hover:bg-success text-white flex items-center gap-1 transition-colors">
          <Download size={11} /> XLS
        </button>
        <button onClick={() => boqAggregator.exportToPDF(boqData, costBreakdown)}
          className="px-2.5 py-1 rounded text-xs font-semibold bg-danger hover:bg-danger text-white flex items-center gap-1 transition-colors">
...
const BOQGenerator = ({ onClose }) => {
  ...
  if (!boqData && !showModal) {
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-gradient-to-br from-black via-surface to-black rounded-lg border-2 border-info/20 min-h-[140px] ">
        <div className="bg-info/10 p-3 rounded-full mb-3">
          <FileText className="w-6 h-6 text-info" />
        </div>
        <h3 className="text-sm font-semibold text-ink mb-1">Bill of Quantities</h3>
        <p className="text-xs text-dim mb-4">Click to generate a professional BOQ</p>
        <button onClick={() => setShowModal(true)}
          className="px-5 py-2 bg-black text-white text-xs font-semibold rounded-lg hover:from-black hover:to-black transition-all ">
          Generate BOQ
        </button>
      </div>
    );
