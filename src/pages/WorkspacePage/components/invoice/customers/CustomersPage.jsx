    ...
    <div className="p-8 bg-gradient-to-br from-surface-hover to-surface-hover min-h-full">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-ink mb-2">Active Customers</h1>
            <p className="text-dim">Manage your customer relationships and track their activity</p>
          </div>
          <div className="flex items-center space-x-3">
            <button className="bg-surface border border-line text-ink px-4 py-2 rounded-lg hover:bg-canvas transition-all duration-200 flex items-center space-x-2 ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-surface p-4 rounded-xl  border border-line  transition-shadow">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm text-dim">Total Customers</p>
                <p className="text-xl font-bold text-ink">{customersData.length}</p>
              </div>
            </div>
          </div>
          <div className="bg-surface p-4 rounded-xl  border border-line  transition-shadow">
          ...
          <div className="bg-surface p-4 rounded-xl  border border-line  transition-shadow">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm text-dim">Total Receivables</p>
          ...
          <div className="bg-surface p-4 rounded-xl  border border-line  transition-shadow">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm text-dim">Credits</p>
                <p className="text-xl font-bold text-ink">₹0.00</p>
              </div>
            </div>
          </div>
      ...
      <div className="bg-surface rounded-2xl shadow-lg overflow-hidden border border-line">
        ...
        <div className="bg-gradient-to-r from-surface-hover to-surface-hover px-6 py-4 border-b border-line">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-ink">Customer Directory</h3>
            <div className="flex items-center space-x-3">
        ...
        {loading && (
          <div className="flex items-center justify-center py-12">
            ...
            <span className="ml-2 text-dim">Loading customers...</span>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="bg-danger/10 border border-danger/20 rounded-lg p-4 mb-6">
            <div className="flex items-center">
              <div className="text-danger mr-3">⚠️</div>
              <div>
                <h3 className="text-danger font-medium">Error loading customers</h3>
                <p className="text-danger text-sm mt-1">{error}</p>
                <button 
                  onClick={fetchCustomers}
                  className="mt-2 text-sm bg-danger text-white px-3 py-1 rounded hover:bg-danger transition-colors"
                >
                  Retry
                </button>
              </div>
          ...
          <div className="overflow-x-auto">
                ...
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-dim uppercase tracking-wider border-b border-line">
                    <div className="flex items-center space-x-1">
                      <span>Customer</span>
                      <svg className="w-3 h-3 text-dim" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <tbody className="divide-y divide-line">
              {customersData.map((customer, index) => (
                <tr key={customer.id} className="hover:bg-gradient-to-r hover:from-black hover:to-black cursor-pointer transition-all duration-200 group">
                  <td className="px-6 py-5">
                    <div className="flex items-center">
                      <div className="w-10 h-10 bg-black rounded-full flex items-center justify-center text-white font-semibold text-sm mr-4 ">
                        {customer.name.charAt(0)}
                      </div>
                      <div>
                        <div className="flex items-center">
                          <span className="text-sm font-semibold text-ink group-hover:text-ink transition-colors">
                            {customer.name}
                          </span>
                          <svg className="w-4 h-4 ml-2 text-dim group-hover:text-ink transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <td className="px-6 py-5">
                    <div className="text-sm text-ink font-medium">{customer.companyName}</div>
                  ...
                  <td className="px-6 py-5">
                    <div className="space-y-1">
                      <div className="flex items-center text-sm text-ink">
                        <svg className="w-4 h-4 mr-2 text-dim" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                        {customer.email === '-' ? (
                          <span className="text-dim italic">No email</span>
                        ) : (
                          <span className="text-ink hover:text-ink">{customer.email}</span>
                        )}
                      </div>
                      <div className="flex items-center text-sm text-ink">
                        <svg className="w-4 h-4 mr-2 text-dim" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {customer.workPhone === '-' ? (
                          <span className="text-dim italic">No phone</span>
                        ) : (
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="text-sm font-semibold text-ink">{customer.receivables}</div>
                    <div className="text-xs text-dim mt-1">Outstanding</div>
                  </td>
                  <td className="px-6 py-5 text-right">
                    <div className="text-sm font-semibold text-ink">{customer.unusedCredits}</div>
                    <div className="text-xs text-dim mt-1">Available</div>
                  </td>
      ...
      {showDeleteConfirm && selectedCustomer && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-surface rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <h3 className="text-lg font-semibold text-ink text-center mb-2">Delete Customer</h3>
            ...
            <div className="flex space-x-3">
              <button
                ...
                className="flex-1 px-4 py-2 text-ink bg-surface-hover hover:bg-surface-hover rounded-lg transition-colors"
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-danger text-white hover:bg-danger rounded-lg transition-colors disabled:opacity-50"
                disabled={actionLoading}
              >
