      ...
      <div className="bg-surface rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-line">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-ink">Configure Turnkey Workflow</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-ink">Task Information</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-ink mb-2">Task Name</label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-ink mb-2">Human Resources</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-ink mb-2">Resources Count</label>
                <input
                  type="number"
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium text-ink">Status Badges</h3>
              <button
                onClick={addStatusBadge}
                className="flex items-center space-x-2 px-3 py-2 bg-info text-white rounded-lg hover:bg-info transition-colors text-sm"
              >
            ...
            <div className="space-y-3">
              {taskData.statusBadges.map((badge) => (
                <div key={badge.id} className="flex items-center space-x-3 p-3 bg-canvas rounded-lg">
                  <input
                    type="text"
                    value={badge.name}
                    onChange={(e) => handleStatusBadgeChange(badge.id, 'name', e.target.value)}
                    className="flex-1 px-3 py-2 border border-line rounded-lg focus:ring-2 focus:ring-info focus:border-transparent"
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium text-ink">Test Cases</h3>
              <button
                onClick={addTestCase}
                className="flex items-center space-x-2 px-3 py-2 bg-info text-white rounded-lg hover:bg-info transition-colors text-sm"
              >
            ...
            <div className="space-y-4">
              {taskData.testCases.map((testCase) => (
                <div key={testCase.id} className="border border-line rounded-lg p-4 bg-surface">
                  {/* Test Case Header */}
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-md font-medium text-ink">Test Case Details</h4>
                    <button
                  ...
                  <div className="border-t border-line pt-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-ink">Evidence Files ({Array.isArray(testCase.evidenceFiles) ? testCase.evidenceFiles.length : 0})</span>
                      <label className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg transition-colors text-sm ${
                        uploadLoading 
                          ? 'bg-canvas text-dim cursor-not-allowed' 
                          : 'bg-info/10 text-info hover:bg-info/10 cursor-pointer'
                      }`}>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {testCase.evidenceFiles.map((file) => (
                          <div key={file.id} className="relative group">
                            <div className="flex items-center space-x-2 p-2 bg-canvas rounded-lg border">
                              ...
                              <button
                                ...
                                className="opacity-0 group-hover:opacity-100 p-0.5 text-danger hover:text-danger transition-all"
                              >
                                <X className="w-3 h-3" />
                              </button>
                            </div>
