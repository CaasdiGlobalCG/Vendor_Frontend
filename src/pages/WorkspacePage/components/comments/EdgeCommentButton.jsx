  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`relative flex items-center justify-center w-6 h-6 rounded-full border-2 transition-all duration-200  hover:scale-110 ${
          unresolvedCount > 0
            ? 'bg-info border-info text-white'
            : comments.length > 0
              ? 'bg-success/10 border-success/30 text-success'
              : 'bg-surface border-line text-dim opacity-0 group-hover:opacity-100'
        }`}
        title={unresolvedCount > 0 ? `${unresolvedCount} unresolved comment${unresolvedCount > 1 ? 's' : ''}` : 'Add comment'}
      >
        <MessageCircle className="w-3 h-3" />
        {unresolvedCount > 0 && (
          <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-danger rounded-full text-[8px] font-bold text-white flex items-center justify-center">
            {unresolvedCount}
          </span>
        )}
      </button>
      ...
      {open && (
        <div
          ref={popoverRef}
          className="absolute left-8 top-0 z-50 w-72 bg-surface rounded-lg shadow-xl border border-line overflow-hidden"
          onClick={e => e.stopPropagation()}
        >
          <div className="flex items-center justify-between px-3 py-2 bg-canvas border-b border-line">
            <span className="text-xs font-semibold text-dim">Edge Comment</span>
            <button onClick={() => setOpen(false)} className="p-0.5 text-dim hover:text-dim rounded">
