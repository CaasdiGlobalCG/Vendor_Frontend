    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-surface rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-line">
          <h2 className="text-xl font-semibold text-ink">Progress Review</h2>
          <button onClick={onClose} className="p-2 hover:bg-surface-hover rounded-lg" aria-label="Close">
        ...
        <div className="p-6 space-y-4">
          ...
          {!loading && !error && (
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-medium text-ink mb-2">Task Progress</h3>
                {!projectStatus ? (
                  <p className="text-sm text-dim">No task progress available.</p>
                ) : (
                  <div className="bg-canvas border border-line rounded-lg p-4">
                    ...
                    <p className="text-sm text-dim mt-1">{projectStatus.description}</p>
                    ...
                    {projectStatus.proofOfCompletion && (
                      <div className="mt-3">
                        <a href={projectStatus.proofOfCompletion} target="_blank" rel="noreferrer" className="inline-flex items-center text-sm text-info">
              ...
              <div>
                <h3 className="text-lg font-medium text-ink mb-2">Project Progress</h3>
                {!completionReq ? (
                  <p className="text-sm text-dim">No project completion request submitted.</p>
                ) : (
                  <div className="bg-canvas border border-line rounded-lg p-4">
                    <div className="text-sm text-ink"><strong>Marked Completed:</strong> {completionReq.markCompleted ? 'Yes' : 'No'}</div>
                    <div className="text-sm text-ink mt-2"><strong>Description:</strong></div>
                    <p className="text-sm text-dim mt-1">{completionReq.completionDescription}</p>
                    ...
                    {completionReq.completionFiles && (
                      <div className="mt-3">
                        <a href={completionReq.completionFiles} target="_blank" rel="noreferrer" className="inline-flex items-center text-sm text-info">
