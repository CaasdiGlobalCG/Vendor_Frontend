...
export default function PortfolioEditor({ content, onChange, accentColor = '#F5A623' }) {
  ...
  const renderTextInput = (label, fieldKey, placeholder, maxLen) => (
    <div className="mb-4">
      ...
      />
      {maxLen && (
        <p className="text-[10px] text-dim mt-1">{(content[fieldKey] || '').length}/{maxLen}</p>
      )}
    </div>
  );
  ...
  const renderTextArea = (label, fieldKey, placeholder, rows = 3) => (
    <div className="mb-4">
      ...
      <textarea
        ...
        placeholder={placeholder}
        rows={rows}
        className="w-full px-4 py-2.5 border border-line rounded-lg focus:ring-2 focus:outline-none text-sm resize-none"
        style={{ '--tw-ring-color': accentColor }}
      />
    </div>
  );
  ...
  const renderTab = () => {
    switch (activeTab) {
      case 'cover':
        return (
          <div>
            <p className="text-xs text-dim mb-4">Edit the cover page details that visitors see first.</p>
      ...
      case 'about':
        return (
          <div>
            <p className="text-xs text-dim mb-4">Describe your company, its background and key highlights.</p>
      ...
      case 'values':
        return (
          <div>
            <p className="text-xs text-dim mb-4">Share your company's vision, mission, and core values.</p>
            ...
            {/* Core Values List */}
            <div className="mb-4">
              <label className="block text-sm font-semibold text-ink mb-2">Core Values</label>
                ...
                {(content.coreValues || []).map((val, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-line rounded-lg text-sm focus:ring-2 focus:outline-none"
                      style={{ '--tw-ring-color': accentColor }}
                    />
                    <button onClick={() => removeListItem('coreValues', i)}
                      className="p-2 text-dim hover:text-danger transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
      ...
      case 'whatwedo':
        return (
          <div>
            <p className="text-xs text-dim mb-4">Describe your industry, business type, and market focus.</p>
            ...
            {/* Segments */}
            <div className="mb-4">
              <label className="block text-sm font-semibold text-ink mb-2">Business Segments</label>
              <div className="space-y-2">
            ...
            {/* Certifications */}
            <div className="mb-4">
              <label className="block text-sm font-semibold text-ink mb-2">Certifications</label>
              <div className="space-y-2">
      ...
      case 'services':
        return (
          <div>
            <p className="text-xs text-dim mb-4">Add or edit your services. Name and description for each.</p>
            <div className="space-y-3">
              {(content.services || []).map((svc, i) => {
                const name = typeof svc === 'string' ? svc : svc?.name || '';
                const desc = typeof svc === 'object' ? svc?.description || '' : '';
                return (
                  <div key={i} className="p-3 border border-line rounded-lg bg-canvas">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-[10px] font-bold text-dim">{String(i + 1).padStart(2, '0')}</span>
                      <input
                        type="text"
                        value={name}
                        onChange={(e) => updateServiceField(i, 'name', e.target.value)}
                        placeholder="Service name"
                        className="flex-1 px-3 py-1.5 border border-line rounded text-sm focus:ring-2 focus:outline-none"
                        style={{ '--tw-ring-color': accentColor }}
                      />
                      <button onClick={() => removeListItem('services', i)}
                        className="p-1.5 text-dim hover:text-danger transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                    <textarea
                      ...
                      rows={2}
                      className="w-full px-3 py-1.5 border border-line rounded text-xs resize-none focus:ring-2 focus:outline-none"
                      style={{ '--tw-ring-color': accentColor }}
                    />
                  </div>
                );
      ...
      case 'products':
        return (
          <div>
            ...
            <div className="p-4 bg-canvas rounded-lg border border-line text-center">
              <Package size={24} className="mx-auto mb-2 text-dim" />
      ...
      case 'projects':
        return (
          <div>
            ...
            <div className="p-4 bg-canvas rounded-lg border border-line text-center">
              <FolderOpen size={24} className="mx-auto mb-2 text-dim" />
              <p className="text-sm text-dim font-medium">
      ...
      case 'contact':
        return (
          <div>
            <p className="text-xs text-dim mb-4">Override contact details shown on the portfolio's last page.</p>
