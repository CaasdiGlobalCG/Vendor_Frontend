...
export default function ProductsPage({ products, accentColor = '#F5A623' }) {
  ...
  return (
    <div pageTitle="Our Products" className="bg-surface relative overflow-hidden" style={{ minHeight: '1123px' }}>
      {/* ===== HEADER ===== */}
      <div className="relative overflow-hidden">
        <div className="px-10 md:px-14 pt-10 pb-8 relative z-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-dim">Our Portfolio</span>
          </div>
          <div className="flex items-end justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-[9px] text-dim font-bold uppercase tracking-wider">Products</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-10 md:px-14 pb-8">
        {productList.length > 0 ? (
            ...
            {productList.slice(0, 6).map((product, i) => {
              ...
              return (
                <div key={i} className="group cursor-pointer"
                  ...
                  {/* Image area */}
                  <div className="relative h-36 rounded-sm overflow-hidden mb-3 bg-surface-hover">
                    {image ? (
                      <>
                      ...
                      <div className="w-full h-full flex items-center justify-center relative">
                        ...
                        <Package size={24} className="text-dim" />
                      </div>
                    )}
                    {/* Number badge */}
                    ...
                    {category && (
                      <div className="absolute bottom-2 right-2 px-2 py-0.5 bg-white/90 rounded-sm">
                        <span className="text-[8px] font-bold uppercase tracking-wider text-dim">{category}</span>
                      </div>
                    )}
                  </div>
                  {/* Info */}
                  <h4 className="font-bold text-xs text-ink mb-0.5 group-hover:underline">{name}</h4>
                  {price && (
                  ...
                  {desc && <p className="text-[10px] text-dim mt-1 line-clamp-2">{desc}</p>}
                </div>
              );
            })}
          </div>
          ...
          <div className="text-center py-20 bg-canvas rounded-sm">
            <Package size={32} className="mx-auto mb-3 text-dim" />
            <p className="text-dim text-sm">Product showcase will appear here</p>
          </div>
        )}
      ...
      {selectedProduct && (
        <div className="absolute inset-0 bg-black/60 z-50 flex items-center justify-center p-8"
          ...
          <div className="bg-surface rounded-sm max-w-md w-full max-h-[90%] overflow-y-auto shadow-2xl"
            ...
            <div className="p-6">
              ...
              {selectedProduct.category && (
                <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm bg-surface-hover mb-3">
                  <Tag size={8} className="text-dim" />
                  <span className="text-[9px] font-bold uppercase text-dim">{selectedProduct.category}</span>
                </div>
              )}
